...
module.exports.get_event_list = function(user, event_type) {
    ...
    if (user.role > 5) {
        ...
        listEventForConsole(event_type).then(function(data) {
            deferred.resolve({
                data: data.data,
                message: data.message
            });
...
function listEventsForMerchant(user, event_type) {
    ...
    Event.find({
        ...
        if (err || !events) {
            deferred.reject({
                err: err || null,
                message: "Unable to retrieve events list"
            });
        } else {
            deferred.resolve({
                data: events,
...
function listEventForConsole(event_type) {
    logger.log();
    var deferred = Q.defer();
    Event.find({
        event_type: event_type
        ...
        if (event.event_meta.status === 'archived') {
            ...
            updateBillStatus(event)
                ...
                .then(function(data) {
                    ...
                    payload.body = "Your bill for " + data.event_meta.outlet_name + ' dated ' + data.event_meta.bill_date +
